...
// // Sink to keep the compiler from eliminating the encode-only microbench (Block A).
// // Updated with cellZoneIndex.size() so the work has an observable side effect.
// static volatile unsigned char sinkChar = 0;
...
template<class ThermoType>
Foam::scalarField Foam::CCMchemistryModel<ThermoType>::maxYstdNorm() const
{
    ...
    reduce(Ymaxvalue, maxOp<scalarList>());
...
template<class ThermoType>
void Foam::CCMchemistryModel<ThermoType>::mpAlltoallvBytes
(
    int N,
...
template<class ThermoType>
void Foam::CCMchemistryModel<ThermoType>::distributeReactionEntry
(
...
template<class ThermoType>
void Foam::CCMchemistryModel<ThermoType>::redistributeLoads()
{
