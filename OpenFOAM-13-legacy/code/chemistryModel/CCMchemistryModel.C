...
:
    ...
    mixture_(dynamicCast<const multicomponentMixture<ThermoType>>(this->thermo())),
    ...
    preAllocatedReceivedRREntries_()
{
    ...
    forAll(principalVars_, pi)
    {
        ...
        if (thermo.species().found(varName))
        {
...
template<class ThermoType>
Foam::tmp<Foam::DimensionedField<Foam::scalar, Foam::volMesh>>
Foam::CCMchemistryModel<ThermoType>::reactionRR
(
...
Foam::CCMchemistryModel<ThermoType>::specieReactionRR
(
    const label reactioni
) const
{
...
)
{
    ...
    std::sort(sortedIndices.begin(), sortedIndices.end(),
              [&](label a, label b) { 
                  label speciesIdxA = thermo().species()[speciesPrincipalVars_[a]];
                  label speciesIdxB = thermo().species()[speciesPrincipalVars_[b]];
    ...
    forAll(sortedIndices, i)
    {
        ...
        label speciesIdx = thermo().species()[speciesName];
...
)
{
    ...
    //tmp<volScalarField> trhovf(this->thermo().rho());
    const volScalarField& rhovf(rho0vf);
...
{
    if (mesh().dynamic() || mesh().distributing() || mesh().topoChanging())
    {
